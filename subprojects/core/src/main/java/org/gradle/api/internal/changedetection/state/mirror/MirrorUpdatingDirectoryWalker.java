    ...
    public ImmutablePhysicalDirectorySnapshot walkDir(final Path rootPath, @Nullable PatternSet patterns) {
        ...
        try {
            Files.walkFileTree(rootPath, EnumSet.of(FileVisitOption.FOLLOW_LINKS), Integer.MAX_VALUE, new java.nio.file.FileVisitor<Path>() {
                ...
                private boolean isAllowed(Path path, boolean isDirectory, @Nullable BasicFileAttributes attrs, Iterable<String> relativePath) {
                    if (spec == null) {
                        return true;
                    }
                    return spec.isSatisfiedBy(new PathBackedFileTreeElement(path, isDirectory, attrs, relativePath, fileSystem));
                }
            });
        } catch (IOException e) {
